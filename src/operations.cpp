...
static int do_lookup(fuse_ino_t parent, const char *name, fuse_entry_param *e)
{
	...
	if (inode.fd > 0)
	{
		debug_print("lookup(): inode {} (userspace) already known; fd = {}", e->attr.st_ino, inode.fd);
		inode.nlookup++;
		fs_lock.unlock();
	}
	else
...
static void sfs_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr, int valid, struct fuse_file_info *fi)
{
	Inode &inode = get_inode(ino);
	int ifd = inode.fd;
	int res = 0;
...
static void sfs_mknod(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode, dev_t rdev)
{
...
static void do_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t offset, fuse_file_info *fi, const int plus)
{
...
static void sfs_readdir(fuse_req_t req, fuse_ino_t ino, size_t size, off_t offset, fuse_file_info *fi)
{
	do_readdir(req, ino, size, offset, fi, 0);
}

static void sfs_readdirplus(fuse_req_t req, fuse_ino_t ino, size_t size, off_t offset, fuse_file_info *fi)
{
...
static void sfs_create(fuse_req_t req, fuse_ino_t parent, const char *name, mode_t mode, fuse_file_info *fi)
{
	Inode &inode_p = get_inode(parent);
...
static void sfs_open(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi)
{
	...
	int fd = with_fd_path(inode.fd, [fi](const char *buf)
...
static void sfs_write_buf(fuse_req_t req, fuse_ino_t ino, fuse_bufvec *in_buf, off_t off, fuse_file_info *fi)
{
	Inode &inode = get_inode(ino);
	if (fs.passthrough && !fs.direct_io)
	{
...
static void sfs_fallocate(fuse_req_t req, fuse_ino_t ino, int mode, off_t offset, off_t length, fuse_file_info *fi)
{
	Inode &inode = get_inode(ino);
...
static void sfs_setxattr(fuse_req_t req, fuse_ino_t ino, const char *name, const char *value, size_t size, int flags)
{
	Inode &inode = get_inode(ino);
	std::string procname = std::format("/proc/self/fd/{}", inode.fd);
